...
#include <windows.h>///> Sleep() used to test the execution time in dynamic since it's always 0 .
...
{///\fn int dynamic_method()
    ...
    /// Calculate the owed sums and build chains.
    ///
    for (i = 0; i <= coins_no; i++)
    {
        for (j = 0; j <= owed; j++)
    ...
    *solution = calloc(coins_no, sizeof(int));
    ...
    else
    {
   	...
   	/// Free the used memory for the solution
   	///
    free(solution);
    /// Free the table
    ///
    ...
    free(table);
    time_h = clock() - time_h;
    double time_taken = ((double) time_h) / CLOCKS_PER_SEC;
    *time_dyn = time_taken ;
...
{///\fn void greedy_method()
	...
	{      
		...
		printf("\nRemainder value that could not be changed is :%d\n",owed);
	}

	time_h = clock() - time_h;
    double time_taken = ((double) time_h) / CLOCKS_PER_SEC;
    printf("Execution time of the greedy function: %lf\n", time_taken);
}